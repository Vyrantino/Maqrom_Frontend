...
export const getCard = async (setCard, idCard) => {
  idCard &&
    (await axios.get(apiURL + "cards/id/" + idCard).then((response) => {
...
/* Photos */
export const uploadCompressedPhoto = async (
  file,
  alt,
  gallery,
  setImage,
...
export const uploadPhoto = async (selectedFile, alt, gallery) => {
  ...
  await axios.post(apiURL, fd).then((response) => console.log(response));
};

export const getAllImages = async (setImages, gallery) => {
  const getUrl = gallery
    ? `${apiURL}images/gallery/${gallery}`
    : `${apiURL}images/`;
  await axios
    .get(getUrl)
    .then((response) => {
...
export const getPaginatedImages = async (
  ...
  const pagesUrl = gallery
    ? `${apiURL}images/gallery/${gallery}`
    : `${apiURL}images/`;
  await axios.get(pagesUrl).then((response) => {
    let responseLength = response.data.length;
    let division = responseLength / 8;
    let decimal = (division % 1) * 10;
    let pageCount = Math.round(responseLength / 8);
    if (decimal === 0) pageCount = pageCount;
    else if (decimal < 5) pageCount = pageCount + 1;
    else pageCount = pageCount;
    setPageCount(pageCount);
  });

  const getUrl = gallery
    ? `${apiURL}images/gallery/${gallery}`
    : `${apiURL}images?limit=8&offset=${(page - 1) * 8}`;
  await axios
    .get(getUrl)
    .then((response) => {
      const allImages = response.data;
...
export const getCarouselItem = async (setCarouselItem, idCarouselItem) => {
  await axios
    .get(apiURL + "carouselItems/id/" + idCarouselItem)
    .then((response) => {
...
export const deleteArticle = async (articleName) => {
  ...
  if (confirm("Esta seguro que quiere borrar el Articulo?")) {
    if (confirm("Esta REALMENTE SEGURO que quiere borrar el Articulo?")) {
      await axios
        .delete(apiURL + "articles/" + encodedArticleName)
...
export const getArticleCarouselItems = async (
  setArticleCarouselItems,
  article
) => {
  const encodedArticle = encodeURIComponent(article);
...
export const getArticleCards = async (setArticleCards, article) => {
  article &&
    (await axios
      .get(apiURL + "cards/article/" + article)
      .then((response) => {
        const allCards = response.data;
...
export const getPapers = async (setPapers, route, article) => {
  ...
  const urlExtension = !article
    ? `route/${encodedParam}`
    : `article/${encodedParam}`;
...
export const getPaper = async (setPaper, idPaper) => {
  ...
  await axios.get(getUrl).then((response) => setPaper(response.data));
};
