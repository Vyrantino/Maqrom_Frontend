...
export default function GalleryPicker(props) {
  ...
  const handleCreateGallery = async (entry) => {
    await createGallery(entry).then(() => {
  ...
  const handleEditGallery = async () => {
    await patchGallery(props.gallery, galleryName).then(() =>
      props.setGallery(galleryName)
    );
  };

  const handleDeleteGallery = async () => {
    await deleteGallery(props.gallery).then(() => props.setGallery(""));
  };
    ...
    <Container maxWidth="sm">
      <FormControl variant="filled" sx={{ m: 1, minWidth: "60%" }}>
        ...
        <Select
          ...
          defaultValue=""
        >
          <MenuItem key={0} value={"Todas las imagenes"}>
            {" "}
            Todas las imagenes{" "}
          </MenuItem>
          {props.galleries.map((item) => (
            <MenuItem key={item.idGallery} value={item.galleryName}>
              {" "}
              {item.galleryName}{" "}
            </MenuItem>
          ))}
        ...
        >
          ...
          >
            ...
            <Button variant="outlined" onClick={handleEditGallery}>
              {" "}
              Editar{" "}
            </Button>
            <Button variant="outlined" onClick={handleDeleteGallery}>
              {" "}
              Borrar{" "}
            </Button>
          </ButtonGroup>
